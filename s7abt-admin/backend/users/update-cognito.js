...
exports.handler = async (event) => {
  ...
  try {
    ...
    const hasCognito = !!(existingUser.cognitoId);
    const cognitoUsername = existingUser.cognitoId || existingUser.email;
      ...
      // Step 1: Update Cognito user attributes (only if user has Cognito account)
      if (hasCognito) {
        console.log('Updating Cognito user:', cognitoUsername);
        ...
        console.log('User has no Cognito account, skipping Cognito operations');
      }
