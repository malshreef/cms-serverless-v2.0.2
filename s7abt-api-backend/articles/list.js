...
exports.handler = async (event) => {
  ...
  try {
    ...
    let query = `
      ...
      WHERE a.s7b_article_active = 1 AND a.s7b_article_deleted_at IS NULL     `;
    ...
    // Execute query using rawQuery for dynamic SQL with LIMIT/OFFSET
    const articles = await db.rawQuery(query, params);
    ...
    // Get total count
    let countQuery = 'SELECT COUNT(*) as total FROM s7b_article a WHERE a.s7b_article_active = 1 AND a.s7b_article_deleted_at IS NULL';
    const countParams = [];
    ...
    const countResult = await db.rawQuery(countQuery, countParams);
