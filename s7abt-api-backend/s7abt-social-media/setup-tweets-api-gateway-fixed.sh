...
# Usage: ./setup-tweets-api-gateway-fixed.sh
...
create_resource_if_not_exists() {
    ...
    if [ -n "$existing_id" ]; then
        >&2 echo "  ✓ Resource '$path_part' already exists (ID: $existing_id)"
        echo "$existing_id"
    else
        >&2 echo "  Creating resource '$path_part'..."
        ...
        >&2 echo "  ✓ Created resource '$path_part' (ID: $new_id)"
        echo "$new_id"
    fi
}
...
TWEETS_RESOURCE_ID=$(create_resource_if_not_exists "$ADMIN_RESOURCE_ID" "tweets")
echo ""
...
TWEET_ID_RESOURCE_ID=$(create_resource_if_not_exists "$TWEETS_RESOURCE_ID" "{id}")
echo ""
...
PUBLISH_RESOURCE_ID=$(create_resource_if_not_exists "$TWEET_ID_RESOURCE_ID" "publish")
echo ""
...
echo "  2. Refresh your admin dashboard (Ctrl+F5)"
