...
exports.handler = async (event) => {
  ...
  try {
    ...
    const countResult = await db.query(
      'SELECT COUNT(*) as total FROM s7b_article WHERE s7b_section_id = ? AND s7b_article_active = 1 AND s7b_article_deleted_at IS NULL',
      [sectionId]
    );
    const total = countResult[0].total;

    // Get articles in this section (use rawQuery for LIMIT/OFFSET)
    const articles = await db.rawQuery(
      ...
      WHERE a.s7b_section_id = ? AND a.s7b_article_active = 1 AND a.s7b_article_deleted_at IS NULL
