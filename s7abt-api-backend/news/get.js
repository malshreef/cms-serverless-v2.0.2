const db = require('../shared/db');
const { success, error } = require('../shared/response');

exports.handler = async (event) => {
  console.log('Event:', JSON.stringify(event));

  try {
    // Get news ID from path parameters
    const newsId = event.pathParameters?.id;

    if (!newsId) {
      return error('News ID is required', 400);
    }

    // Get news details with explicit column aliases (consistent with list endpoint)
    const newsResult = await db.rawQuery(
      `SELECT
        s7b_news_id as id,
        s7b_news_title as title,
        s7b_news_brief as brief,
        s7b_news_body as body,
        s7b_news_image as image,
        s7b_news_logo as logo,
        s7b_news_add_date as createdAt,
        s7b_user_id as userId
      FROM s7b_news
      WHERE s7b_news_active = 1 AND s7b_news_deleted_at IS NULL AND s7b_news_id = ?`,
      [newsId]
    );

    if (newsResult.length === 0) {
      return error('News not found', 404);
    }

    const newsItem = newsResult[0];

    // Get tags for this news
    const tags = await db.query(
      `
      SELECT
        t.s7b_tags_id as id,
        t.s7b_tags_name as name
      FROM s7b_tags t
      INNER JOIN s7b_tags_item ti ON t.s7b_tags_id = ti.s7b_tags_id
      WHERE ti.s7b_news_id = ?
      `,
      [newsId]
    );

    newsItem.tags = tags;

    // Note: News doesn't have comments in this system
    // Comments are only for articles

    return success({ news: newsItem });

  } catch (err) {
    console.error('Error:', err);
    return error('Failed to fetch news', 500, err.message);
  }
};
